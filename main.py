...
async def Glcomads(ctx):
    await ctx.send(f' Мои команды:?Globaltempkr ?Globaltempthen ?Globaltempresson и ?Globaltempimg :nerd:.!')
@bot.command()
...
async def Globaltempresson(ctx):
    await ctx.send(f' К последствиям глобального потепления можно отнести:Таяние арктических льдов; Повышение уровня океана; Отступление ледников; Природные катаклизмы (засухи, ураганы, наводнения); Волны тепла и другие квазистационарные погодные состояния; Закисление и деоксигенация океана; Резкое изменение климата:nerd:!')
@bot.command()
